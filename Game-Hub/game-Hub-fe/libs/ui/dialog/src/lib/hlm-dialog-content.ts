...
@Component({
	...
	template: `
		...
		@if (showCloseButton()) {
			<button hlmBtn variant="ghost" size="icon-sm" class="!border-0 !bg-transparent !text-[#9ca3af] !text-[28px] !cursor-pointer
              transition duration-200 ease-in-out
              hover:!text-white hover:!scale-110
              absolute end-2 top-2" hlmDialogClose>
...
export class HlmDialogContent {
	...
	constructor() {
		classes(() => ['bg-[#10131a] text-[#ddd] ' +
    'border border-[rgba(255,255,255,0.12)] rounded-[18px] ' +
    'data-open:animate-in data-closed:animate-out ' +
    'data-closed:fade-out-0 data-open:fade-in-0 ' +
    'data-closed:zoom-out-95 data-open:zoom-in-95 ' +
    'grid max-w-[calc(100%-2rem)] gap-4 p-4 text-sm ' +
    'duration-100 sm:max-w-sm relative mx-auto w-full outline-none sm:mx-0',
